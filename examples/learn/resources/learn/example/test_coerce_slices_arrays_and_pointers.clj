...
(az/deftest constant-array-to-slice-test
  (let [literal (ak/as "hello" [:slice-const :u8])
        letters
        (ak/as (& (az/array-init [:array 5 :u8] [\h \e \l \l 111])) [:slice-const :u8])
        numbers
        (ak/as (& (az/array-init [:array 2 :f32] [1.2 3.4])) [:slice-const :f32])]
...
(az/deftest constant-array-to-error-slice-test
  (let [literal (ak/as "hello" [:error-union :anyerror [:slice-const :u8]])
        letters
        (ak/as (& (az/array-init [:array 5 :u8] [\h \e \l \l 111])) [:error-union :anyerror [:slice-const :u8]])
        numbers
        (ak/as (& (az/array-init [:array 2 :f32] [1.2 3.4])) [:error-union :anyerror [:slice-const :f32]])]
...
(az/deftest constant-array-to-optional-slice-test
  (let [literal (ak/as "hello" [:optional [:slice-const :u8]])
        letters
        (ak/as (& (az/array-init [:array 5 :u8] [\h \e \l \l 111])) [:optional [:slice-const :u8]])
        numbers
        (ak/as (& (az/array-init [:array 2 :f32] [1.2 3.4])) [:optional [:slice-const :f32]])]
...
(az/deftest array-to-slice-test
  ...
  (let [^:var buffer (ak/as (deref "hello") [:array 5 :u8])
