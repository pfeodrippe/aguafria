...
(az/deftest constant-array-to-slice-test
  (let [literal (k/as "hello" [:slice-const :u8])
        letters
        (k/as (k/& (az/array-init [\h \e \l \l 111] [:array 5 :u8])) [:slice-const :u8])
        numbers
        (k/as (k/& (az/array-init [1.2 3.4] [:array 2 :f32])) [:slice-const :f32])]
...
(az/deftest constant-array-to-error-slice-test
  (let [literal (k/as "hello" [:error-union :anyerror [:slice-const :u8]])
        letters
        (k/as (k/& (az/array-init [\h \e \l \l 111] [:array 5 :u8])) [:error-union :anyerror [:slice-const :u8]])
        numbers
        (k/as (k/& (az/array-init [1.2 3.4] [:array 2 :f32])) [:error-union :anyerror [:slice-const :f32]])]
...
(az/deftest constant-array-to-optional-slice-test
  (let [literal (k/as "hello" [:optional [:slice-const :u8]])
        letters
        (k/as (k/& (az/array-init [\h \e \l \l 111] [:array 5 :u8])) [:optional [:slice-const :u8]])
        numbers
        (k/as (k/& (az/array-init [1.2 3.4] [:array 2 :f32])) [:optional [:slice-const :f32]])]
...
(az/deftest array-to-slice-test
  ...
  (let [buffer (k/var (deref "hello") [:array 5 :u8])
    ...
    (try (testing/expectEqualSlices
          (az/type :f32)
          (k/& (az/array-init [1.2 3.4] [:array 2 :f32]))
          values))))
