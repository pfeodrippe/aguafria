(ns simple-game.physics
  "Bounded native Box3D particles for coco-factory events."
  (:require [aguafria.std]
            [aguafria.keyword :as ak]
            [aguafria.std.math :as std-math]
            [aguafria.std.mem :as std-mem]
            [aguafria.zig :as az]
            [simple-game.bindings]
            [simple-game.bindings.box3d :as box3d]))

(az/defconst particle-capacity :usize 64)

(az/defstruct Particle
  "One pooled visual particle backed by a real Box3D body."
  {:layout :extern}
  [[:body box3d/b3BodyId]
   [:age :f32]
   [:radius :f32]
   [:event_kind :u8]
   [:active :bool]])

(az/defstruct ParticleView
  "Exact Box3D state consumed by rendering and nREPL inspection."
  {:layout :extern}
  [[:active :bool]
   [:x :f32]
   [:y :f32]
   [:z :f32]
   [:radius :f32]
   [:age :f32]
   [:event_kind :u8]])

(az/defstruct PhysicsSnapshot
  "Inspectable native Box3D world and bounded particle-pool state."
  {:layout :extern}
  [[:initialized :bool]
   [:active_particles :u32]
   [:capacity :u32]
   [:emitted :u64]
   [:box3d_bytes :i32]])

(az/defvar initialized false)

(az/defvar world box3d/b3WorldId
  (std-mem/zeroes (az/type box3d/b3WorldId)))

(az/defvar particles [:array 64 Particle]
  (std-mem/zeroes (az/type [:array 64 Particle])))

(az/defvar next-slot :usize 0)

(az/defvar emitted-count :u64 0)

(az/defn initialize!
  "Create one Box3D world; compatible code reloads preserve its live bodies."
  :-
  :bool
  []
  (when (ak/! initialized)
    (let [^{:var true} definition (box3d/b3DefaultWorldDef)]
      (set! (az/field definition gravity)
            (box3d/b3Vec3 {:x 0.0 :y -9.8 :z 0.0}))
      (set! (az/field definition workerCount) 1)
      (set! world (box3d/b3CreateWorld (ak/& definition)))
      (set! particles (std-mem/zeroes (az/type [:array 64 Particle])))
      (set! next-slot 0)
      (set! emitted-count 0)
      (set! initialized true)))
  initialized)

(az/defn destroy-slot!
  :-
  :void
  [[slot :usize]]
  (let [particle (ak/& (az/index particles slot))]
    (when (az/field (az/deref particle) active)
      (box3d/b3DestroyBody (az/field (az/deref particle) body))
      (set! (az/field (az/deref particle) active) false))))

(az/defn spawn-one!
  "Spawn one coconut fragment at a factory grid location."
  :-
  :void
  [[grid-x :i32]
   [grid-y :i32]
   [ordinal :usize]
   [event-kind :u8]]
  (set! _ (initialize!))
  (let [slot next-slot
        ordinal-f (ak/as :f32 (ak/floatFromInt ordinal))
        angle (* ordinal-f 0.7853982)
        speed (+ 1.8 (* ordinal-f 0.12))
        radius (+ 0.08 (* 0.01 (ak/as :f32 (ak/floatFromInt (mod ordinal 3)))))
        world-x (* (- (ak/as :f32 (ak/floatFromInt grid-x)) 11.0) 0.5)
        world-z (* (- (ak/as :f32 (ak/floatFromInt grid-y)) 12.0) 0.5)
        ^{:var true} body-definition (box3d/b3DefaultBodyDef)
        ^{:var true} shape-definition (box3d/b3DefaultShapeDef)
        sphere (box3d/b3Sphere
                {:center (box3d/b3Vec3 {:x 0.0 :y 0.0 :z 0.0})
                 :radius radius})]
    (destroy-slot! slot)
    (set! (az/field body-definition type) box3d/b3_dynamicBody)
    (set! (az/field body-definition position)
          (box3d/b3Pos {:x world-x :y 0.35 :z world-z}))
    (set! (az/field body-definition linearVelocity)
          (box3d/b3Vec3
           {:x (* (std-math/cos angle) speed)
            :y (+ 2.8 (* ordinal-f 0.10))
            :z (* (std-math/sin angle) speed)}))
    (set! (az/field body-definition angularVelocity)
          (box3d/b3Vec3 {:x (* 0.7 ordinal-f)
                         :y 1.4
                         :z (- 2.4 (* 0.3 ordinal-f))}))
    (set! (az/field body-definition linearDamping) 0.08)
    (set! (az/field body-definition angularDamping) 0.12)
    (set! (az/field body-definition enableSleep) false)
    (set! (az/field shape-definition density) 0.7)
    (set! (az/field (az/field shape-definition baseMaterial) friction) 0.25)
    (set! (az/field (az/field shape-definition baseMaterial) restitution) 0.38)
    (let [body (box3d/b3CreateBody world (ak/& body-definition))]
      (set! _ (box3d/b3CreateSphereShape
               body (ak/& shape-definition) (ak/& sphere)))
      (set! (az/index particles slot)
            (Particle {:body body
                       :age 0.0
                       :radius radius
                       :event_kind event-kind
                       :active true})))
    (set! next-slot (mod (+ next-slot 1) particle-capacity))
    (set! emitted-count (+ emitted-count 1))))

(az/defn emit!
  "Emit a bounded physical burst for a real factory event."
  :-
  :void
  [[grid-x :i32]
   [grid-y :i32]
   [event-kind :u8]]
  (let [^{:zig/type :usize} count (if (ak/== event-kind 4) 12 6)]
    (dotimes [ordinal count]
      (spawn-one! grid-x grid-y ordinal event-kind))))

(az/defn step!
  "Advance Box3D and retire particles after their short visual lifetime."
  :-
  :void
  [[delta-seconds :f32]]
  (when initialized
    (let [step (if (> delta-seconds 0.0) delta-seconds 0.008333333)]
      (box3d/b3World_Step world step 4)
      (dotimes [slot particle-capacity]
        (let [particle (ak/& (az/index particles slot))]
          (when (az/field (az/deref particle) active)
            (set! (az/field (az/deref particle) age)
                  (+ (az/field (az/deref particle) age) step))
            (when (> (az/field (az/deref particle) age) 1.6)
              (destroy-slot! slot))))))))

(az/defn particle-view
  "Return one particle's current native position."
  :-
  ParticleView
  [[slot :usize]]
  (if (or (>= slot particle-capacity)
          (ak/! (az/field (az/index particles slot) active)))
    (ParticleView {:active false
                   :x 0.0 :y 0.0 :z 0.0
                   :radius 0.0 :age 0.0 :event_kind 0})
    (let [particle (ak/& (az/index particles slot))
          position (box3d/b3Body_GetPosition
                    (az/field (az/deref particle) body))]
      (ParticleView
       {:active true
        :x (az/field position x)
        :y (az/field position y)
        :z (az/field position z)
        :radius (az/field (az/deref particle) radius)
        :age (az/field (az/deref particle) age)
        :event_kind (az/field (az/deref particle) event_kind)}))))

(az/defn fill-active-views!
  "Copy the active pool into one contiguous renderer buffer in one native call."
  :-
  :usize
  [[output [:c-pointer ParticleView]]]
  (let [^{:var true :zig/type :usize} count 0]
    (dotimes [slot particle-capacity]
      (let [particle (ak/& (az/index particles slot))]
        (when (az/field (az/deref particle) active)
          (let [position (box3d/b3Body_GetPosition
                          (az/field (az/deref particle) body))]
            (set! (az/index output count)
                  (ParticleView
                   {:active true
                    :x (az/field position x)
                    :y (az/field position y)
                    :z (az/field position z)
                    :radius (az/field (az/deref particle) radius)
                    :age (az/field (az/deref particle) age)
                    :event_kind (az/field (az/deref particle) event_kind)}))
            (set! count (+ count 1))))))
    count))

(az/defn active-count
  :-
  :u32
  []
  (let [^{:var true :zig/type :u32} count 0]
    (dotimes [slot particle-capacity]
      (when (az/field (az/index particles slot) active)
        (set! count (+ count 1))))
    count))

(az/defn snapshot
  "Inspect Box3D allocation and particle occupancy from Clojure."
  :-
  PhysicsSnapshot
  []
  (PhysicsSnapshot
   {:initialized initialized
    :active_particles (active-count)
    :capacity (ak/intCast particle-capacity)
    :emitted emitted-count
    :box3d_bytes (box3d/b3GetByteCount)}))

(az/defn shutdown!
  "Destroy the Box3D world and every body it owns."
  :-
  :void
  []
  (when initialized
    (box3d/b3DestroyWorld world)
    (set! particles (std-mem/zeroes (az/type [:array 64 Particle])))
    (set! world (std-mem/zeroes (az/type box3d/b3WorldId)))
    (set! next-slot 0)
    (set! initialized false)))
