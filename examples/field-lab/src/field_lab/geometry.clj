...
(az/defn vector-subtract p/Vec3 [[a p/Vec3] [b p/Vec3]]
...
(az/defn difference [:array 3 Interval] [[a p/Vec3] [b p/Vec3]]
  (az/init [(subtract (point (az/field a x)) (point (az/field b x)))
...
(az/defn cubic-excludes-zero? :bool [[coefficients [:array 4 Interval]] [depth :u32]]
...
(az/defn non-coplanar? :bool [[before [:pointer {:size :c :const? true} p/Vec3]] [after [:pointer {:size :c :const? true} p/Vec3]]]
  (let [edge-u-before (difference (az/index before 1) (az/index before 0))
        ...
        coefficients (az/init [(multiply (determinant edge-u-before edge-v-before edge-w-before) (point 3.0))
...
(az/defn projected-difference Interval [[a p/Vec3] [b p/Vec3] [axis p/Vec3]]
  (let [^:var result (point 0.0)]
    (dotimes [index 3]
...
(az/defn finite-vector? :bool [[v p/Vec3]]
...
(az/defn pitoco_geometry_ccd_prepare :u32
  ...
  (when (or (> kind 1) (ak/== before null) (ak/== after null) (ak/== error-bound null)
    ...
    (ak/return 3))
  (let [^:var extent (az/init [1.0 1.0 1.0] [:array 3 :f64])]
    (dotimes [index 4]
      (dotimes [axis 3]
...
(az/defn- edge-less? :bool [[context :u8] [left :u64] [right :u64]]
  (set! _ context)
  (< left right))
