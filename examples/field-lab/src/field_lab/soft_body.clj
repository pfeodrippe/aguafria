...
(az/defn solve-volume! :void
  ...
  (let [face (az/index mesh/faces index)
        indices (az/init [0 (az/index face 0) (az/index face 1) (az/index face 2)] [:array 4 :u32])
        ...
        gradients (az/init [ga gb gc gd] [:array 4 p/Vec3])
...
(az/defn project-vertex! :void
  ...
  (let [point (az/index (az/field (az/deref a) positions) vertex)
    ...
    (let [face (az/index mesh/faces closest-face)
          ...
          weights (az/init [(/ weight-x total) (/ weight-y total) (/ weight-z total)] [:array 3 :f64])
