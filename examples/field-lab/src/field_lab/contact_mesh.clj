...
(az/defn feature-sweep CCDResult
  ...
  (let [before (az/init [a0 b0 c0 d0] [:array 4 p/Vec3])
        after (az/init [a1 b1 c1 d1] [:array 4 p/Vec3])
...
(az/defn set-face! :void
  ...
  (set! (az/index (az/field surface faces) index) (az/init [a b c] [:array 3 :u32])))
...
(az/defn triangle-closest Closest
  ...
  (let [vertices (az/init [a b c] [:array 3 p/Vec3])
...
(az/defn near-face-pairs! FacePairsReport
  ...
  (let [^:var report (FacePairsReport {:status 0 :count 0 :visits 0})
        ...
        ^:var size (ak/usize 1)]
    (set! (az/index stack 0) (az/init [0 0] [:array 2 :usize]))
    (while (> size 0)
          ...
          (if (and (az/field left leaf) (az/field right leaf))
            (do
              ...
              (set! (az/index pairs (az/field report count))
                    (az/init [(ak/intCast (az/field left face)) (ak/intCast (az/field right face))] [:array 2 :u32]))
            ...
            (let [split-a (or (az/field right leaf) (ak/! (az/field left leaf)))]
              ...
              (az/set-many!
                (az/index stack size)
                (az/init [(if split-a (az/field left right) ia)
                                                 (if split-a ib (az/field right right))] [:array 2 :usize])
                (az/index stack (+ size 1))
                (az/init [(if split-a (az/field left left) ia)
...
(az/defn sweep-surfaces MeshSweepResult
  ...
  (let [^:var report (MeshSweepResult {:status 2 :visits 0 :queries 0 :candidates 0
    ...
    (az/set-many!
      ...
      (az/index stack 0) (az/init [0 0] [:array 2 :usize]))
    (while (> size 0)
      (when (>= (+ (az/field report visits) (az/field report queries)) maximum-work)
        (az/set-many! (az/field report status) 4 (az/field report time) 0.0)
        (ak/return report))
        ...
        (when (boxes-near? bounds-a bounds-b separation)
          (if (and (az/field node-a leaf) (az/field node-b leaf))
            ...
            (let [span-a (p/add (az/field bounds-a upper) (p/scale (az/field bounds-a lower) -1.0))
              ...
              (az/set-many!
                (az/index stack size)
                (az/init [(if split-a (az/field node-a right) ia)
                                                 (if split-a ib (az/field node-b right))] [:array 2 :usize])
                (az/index stack (+ size 1))
                (az/init [(if split-a (az/field node-a left) ia)
