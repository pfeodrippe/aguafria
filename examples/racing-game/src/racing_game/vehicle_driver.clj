...
(az/defn lane-plan-state [:array 3 :f32]
  ...
  (if (ak/! (az/field plan active))
    (az/init [(az/field plan target) 0.0 0.0] [:array 3 :f32])
    (let [delta (- distance (az/field plan origin))
          ...
          second (/ (+ (* 2.0 (az/index c 2)) (* u (+ (* 6.0 (az/index c 3))
                    ...
                    (* length length))]
      (az/init [position (if (< u 1.0) first 0.0) (if (< u 1.0) second 0.0)] [:array 3 :f32]))))
    ...
    (when (> (ak/abs (- target (az/field plan target))) 0.0001)
      (let [previous (lane-plan-state plan distance)
        ...
        (ak/= (az/field plan coefficients)
          (az/init [(az/index previous 0) slope curvature
