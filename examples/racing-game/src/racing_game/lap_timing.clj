  ...
  (let [^:var result previous]
    (when (and running (ak/! (az/field previous terminal)))
      (when (ak/! (az/field result started))
        (ak/= (az/field result started) true)
        (ak/= (az/field result start_tick) tick)
        ...
        (when (ak/!= lap (az/field result lap))
          (let [elapsed (- tick (az/field result start_tick))
            ...
            (when (and sequential (az/field result complete_start) (> elapsed 0))
              (ak/= (az/field result last_ticks) elapsed)
              (ak/= (az/field result best_ticks)
                    (if (ak/== (az/field result samples) 0) elapsed
                      (ak/min elapsed (az/field result best_ticks))))
              (ak/= (az/field result samples) (+ (az/field result samples) 1)))
            (ak/= (az/field result complete_start) sequential)
            (ak/= (az/field result start_tick) tick)
            (ak/= (az/field result lap) lap)))
        (ak/= (az/field result observed_tick) tick)
        (ak/= (az/field result terminal) terminal)))
    result))
