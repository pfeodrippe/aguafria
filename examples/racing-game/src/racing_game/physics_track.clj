...
(az/defn create! [:* b3/b3MeshData]
  ...
  (let [^:var vertices (mem/zeroes (az/type [:array (* (+ segments 1) track/surface-columns) b3/b3Vec3]))
      ...
      (let [progress (/ (ak/as (ak/floatFromInt i) :f32)
        ...
        (dotimes [lane track/surface-columns]
          (let [p (track/surface-point progress lane)]
            (ak/= (az/index vertices (+ (* i track/surface-columns) lane))
      ...
      (dotimes [lane (- track/surface-columns 1)]
        (let [a (ak/as (ak/intCast (+ (* i track/surface-columns) lane)) :i32)
          ...
          (when (> (- (track/surface-boundary pb (+ lane 1)) (track/surface-boundary pb lane)) 0.001)
            (ak/= (az/index indices (* triangles 3)) a)
            (ak/= (az/index indices (+ (* triangles 3) 1)) (+ a stride))
            (ak/= (az/index indices (+ (* triangles 3) 2)) (+ a stride 1))
            (ak/= (az/index materials triangles) material)
            (ak/= triangles (+ triangles 1)))
          (when (> (- (track/surface-boundary pa (+ lane 1)) (track/surface-boundary pa lane)) 0.001)
            (ak/= (az/index indices (* triangles 3)) a)
            (ak/= (az/index indices (+ (* triangles 3) 1)) (+ a stride 1))
            (ak/= (az/index indices (+ (* triangles 3) 2)) (+ a 1))
            (ak/= (az/index materials triangles) material)
            (ak/= triangles (+ triangles 1))))))
    (ak/= (az/field definition vertices) (ak/& vertices))
    (ak/= (az/field definition indices) (ak/& indices))
    (ak/= (az/field definition materialIndices) (ak/& materials))
    (ak/= (az/field definition vertexCount) (ak/intCast (az/field vertices len)))
    (ak/= (az/field definition triangleCount) (ak/intCast triangles))
    ...
    (ak/= (az/field (az/index surface-materials 0) friction) 0.4)
    (ak/= (az/field (az/index surface-materials 1) friction) 1.0)
    (ak/= (az/field shape materials) (ak/& surface-materials))
    (ak/= (az/field shape materialCount) 2)
