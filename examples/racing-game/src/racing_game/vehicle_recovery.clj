    ...
    (cond
      ...
      (do
        (if (and safe (< speed 0.1) (> front-gap 0.0) (< front-gap 8.0)
          ...
          (ak/= (az/field state waiting_ticks)
                (ak/min 360 (+ (az/field state waiting_ticks) 1)))
          (ak/= (az/field state waiting_ticks) 0))
        (when (>= (az/field state waiting_ticks) 360)
          (ak/= (az/field state phase) 1)
          (ak/= (az/field state start_x) (az/field body x))
          (ak/= (az/field state start_y) (az/field body y))))

      (ak/== (az/field state phase) 1)
      (do
        (ak/= gear -1)
        (ak/= (az/field control steering) (reverse-steering body))
        (ak/= (az/field control throttle) (driver/clamp-unit (* (- 1.2 speed) 0.25)))
        (ak/= (az/field control brake) (driver/clamp-unit (* (- speed 1.2) 0.5)))
      ...
      (do
        ...
        (when (< speed 0.05)
          (ak/= (az/field state phase) (if safe (ak/as 3 :u8) 0))
          (ak/= (az/field state waiting_ticks) 0)
          (ak/= (az/field state start_x) (az/field body x))
          (ak/= (az/field state start_y) (az/field body y))))
      ...
      (do
        ...
        (when (and safe (> (ak/abs (az/field normal steering)) 0.1))
          (ak/= control normal)
          (ak/= (az/field control throttle) (driver/clamp-unit (* (- 2.0 speed) 0.25)))
          (ak/= (az/field control brake) (driver/clamp-unit (* (- speed 2.0) 0.5))))
        (cond
          (> front-gap 18.0)
          (do
      ...
      (ak/== (az/field state phase) 4)
      (do
        (ak/= control (passing-control body (az/field state start_x) (az/field state start_y)))
        (when (< front-gap 6.0)
          (ak/= (az/field control throttle) 0.0)
          (ak/= (az/field control brake) 1.0))
        (ak/= (az/field state waiting_ticks) (ak/min 1800 (+ (az/field state waiting_ticks) 1)))
      ...
      :else (ak/= (az/field state phase) 0))
