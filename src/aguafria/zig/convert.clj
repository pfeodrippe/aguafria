...
(def ^:private development-loader-resource
  "aguafria/development-loader.zig")
  ...
  (loop [remaining (:root-decls context)
    ...
    (if-let [node-index (first remaining)]
      (let [leading (leading-source context previous node-index)
            ...
            form (if (seq? form) (named-container-declaration form) form)]
...
(defn- presentational-form
  ...
  (if-let [attributes-index (declaration-attributes-index form)]
    (let [items (vec form)
      ...
      (if (map? attributes)
        (let [docstring? (string? (nth items (dec attributes-index) nil))
...
(defn- compact-generated-seq
  ...
  (let [items (mapv compact-generated-form form)
        ...
        doc? (and attributes-index (string? (nth items (dec attributes-index) nil)))
...
(defn- compact-generated-form
  ...
  (cond
    ...
    (vector? form)
    (let [items (mapv compact-generated-form form)
          properties (second items)
          ;; Field/tag properties are data, so there is no declaration printer
          ;; to remove an empty converted attribute set for us.
          compact-field? (and (map? properties) (contains? properties :attrs)
                              (empty? (:attrs properties)))
          properties (if compact-field?
                       (dissoc properties :attrs) properties)
          items (if compact-field?
                  (if (empty? properties)
                    (into [(first items)] (drop 2 items))
                    (assoc items 1 properties))
                  items)]
...
(defn- restore-module-imports
  [module-imports namespace-symbol source]
  (let [source
        (reduce
         (fn [source [alias {:keys [namespace import-name]}]]
           (let [zig-alias (emitter/identifier (symbol alias))
             ...
             (if (str/starts-with? (str alias) "module-")
               ...
               source)))
         source
         (get module-imports (str namespace-symbol)))]
...
(defn- restore-source-imports
  ...
  (restore-module-imports (:module-imports report) namespace-symbol source))
...
(defn- module-dependency-closure
  [module-imports roots]
  (loop [pending (vec (map str roots))
         visited #{}]
    (if-let [module (peek pending)]
      (if (contains? visited module)
        (recur (pop pending) visited)
        (let [dependencies
              (->> (get module-imports module)
                   vals
                   (keep :namespace)
                   (map str)
                   (filter #(contains? module-imports %)))]
          (recur (into (pop pending) dependencies)
                 (conj visited module))))
      visited)))
...
(defn materialize-development-project!
  ...
  `entry-module` and `entry-declaration` identify the Zig library's existing
  initialization boundary. Aguafria instruments the converted runtime
  dependency closure, adds its generic Zig publication listener beside the
  entry file, and injects one listener start call into that declaration's
  generated implementation. Native UI/toolkit code continues to call the
  library's original ABI and has no Aguafria integration code.

  The ordinary `materialize-project!` path remains static and is what final
  ...
  ([report-or-path output-root options]
   (let [{:keys [entry-module entry-declaration roots loader-relative-path
         ...
         _ (when-not (and entry-module entry-declaration)
             (throw
              (ex-info
               "Development materialization requires an entry module and declaration"
               {:aguafria/phase :development-materialization
         ...
         files-by-module
         (into {}
               (map (fn [{:keys [namespace] :as file}]
                      [(str namespace) file]))
               (:files report))
         entry-file
         (or (get files-by-module entry-module)
             (throw
              (ex-info "Development entry module is absent from the conversion"
                       {:aguafria/phase :development-materialization
                        :entry-module entry-module})))
         ...
         _ (when-not (.startsWith loader-target (.toPath output-root))
             (throw
              (ex-info "Development loader path escapes the output project"
                       {:aguafria/phase :development-materialization
                        :loader-relative-path loader-relative-path
                        :output-root (.getAbsolutePath output-root)})))
         _ (when (some #(= loader-relative-path (str (:relative-path %)))
                       (:files report))
             (throw
              (ex-info "Development loader path collides with project source"
                       {:aguafria/phase :development-materialization
                        :loader-relative-path loader-relative-path})))
         loader-import
         (-> (.relativize entry-parent
                          (Path/of loader-relative-path
                                   (make-array String 0)))
             str
             (str/replace "\\" "/"))
         closure
         (module-dependency-closure
          ...
          (or (seq roots) [entry-module]))
         missing (sort (remove #(contains? files-by-module %) closure))
         _ (when (seq missing)
             (throw
              (ex-info "Development dependency closure has no converted source"
                       {:aguafria/phase :development-materialization
                        :missing-modules missing})))
         development-files
         (mapv
          (fn [module]
            (let [{:keys [relative-path leading-trivia namespace]}
                  (get files-by-module module)
                  bootstrap? (= module entry-module)
                  source
                  (runtime/development-source
                   ...
                   (when bootstrap?
                     {:bootstrap-declaration entry-declaration
                      :bootstrap-source
                      (str "_ = @import("
                           (emitter/emit-expr loader-import)
                           ").start();")}))
                  source (restore-source-imports report namespace source)
                  source (if (str/blank? leading-trivia)
                           source
                           (if (str/starts-with? source leading-trivia)
                             source
                  ...
                  source (if format?
                           (format-materialized-zig
                            zig (:zig-version report) source output-root
                            relative-path)
                           source)
              ...
              {:module module
               ...
               :bootstrap? bootstrap?
               :bytes (count (.getBytes source StandardCharsets/UTF_8))}))
          (sort closure))
         ...
         loader-source (if format?
                         (format-materialized-zig
                          ...
                          loader-relative-path)
                         loader-source)
         _ (io/make-parents (.toFile loader-target))
         _ (Files/writeString
            loader-target loader-source StandardCharsets/UTF_8
            (into-array StandardOpenOption
                        [StandardOpenOption/CREATE
                         StandardOpenOption/TRUNCATE_EXISTING
                         StandardOpenOption/WRITE]))
         ...
         (assoc base
                :development? true
                :entry-module entry-module
                :entry-declaration (str entry-declaration)
                :loader-relative-path loader-relative-path
                :development-module-count (count development-files)
                :development-files development-files)]
     (record-conversion! result))))

(defn materialize-development-tree!
  "Materialize an in-memory pure-Zig editor tree as a reloadable dev project.

  This is the no-generated-Clojure counterpart of
  `materialize-development-project!`. `load-source-tree!` supplies `tree`;
  ordinary project assets and non-runtime Zig build files are copied into an
  Aguafria-owned output directory, while only the runtime dependency closure
  ...
  (let [started (System/nanoTime)
        ...
        input-path (.toPath input-root)
        output-path (.toPath output-root)
        _ (when (or (= input-root output-root)
                    (.startsWith output-path input-path))
            (throw
             (ex-info "Development output must be outside the input project"
                      {:aguafria/phase :development-materialization
                       :input-root (.getAbsolutePath input-root)
                       :output-root (.getAbsolutePath output-root)})))
        ...
        plans-by-module
        (into {}
              (map (fn [{:keys [namespace] :as plan}]
                     [(str namespace) plan]))
              (:plans tree))
        modules (get-in tree [:catalog :modules])
        module-imports
        (into {}
              (map (fn [[module details]]
                     [(str module) (:imports details)]))
              modules)
        entry-module (str entry-module)
        entry-plan
        (or (get plans-by-module entry-module)
            (throw
             (ex-info "Development entry module is absent from the source tree"
                      {:aguafria/phase :development-materialization
                       :entry-module entry-module})))
        entry-relative (.relativize input-path
                                    (.toPath ^File (:file entry-plan)))
        entry-parent (or (.getParent entry-relative)
                         (Path/of "" (make-array String 0)))
        loader-relative-path
        (or loader-relative-path
            (str (.resolve entry-parent "aguafria_hot_reload.zig")))
        loader-relative-path (str/replace (str loader-relative-path) "\\" "/")
        loader-target (.normalize (.resolve output-path loader-relative-path))
        _ (when-not (.startsWith loader-target output-path)
            (throw
             (ex-info "Development loader path escapes the output project"
                      {:aguafria/phase :development-materialization
                       :loader-relative-path loader-relative-path
                       :output-root (.getAbsolutePath output-root)})))
        _ (when (.startsWith loader-target input-path)
            (throw
             (ex-info "Development loader cannot be written into project source"
                      {:aguafria/phase :development-materialization
                       :loader-relative-path loader-relative-path})))
        loader-import
        (-> (.relativize entry-parent
        ...
        closure
        (module-dependency-closure
         module-imports (or (seq roots) [entry-module]))
        instrumented-modules (set/intersection closure
                                               (set (keys plans-by-module)))
        copied
        (mapv
         (fn [^Path source-path]
           (let [relative (.relativize input-path source-path)
                 target (.normalize (.resolve output-path relative))]
             (when-not (.startsWith target output-path)
               (throw
                (ex-info "Development project path escapes its output root"
                         {:aguafria/phase :development-materialization
                          :relative (str relative)})))
             (io/make-parents (.toFile target))
             (if (Files/isSymbolicLink source-path)
               (let [link (Files/readSymbolicLink source-path)]
                 (Files/deleteIfExists target)
                 (Files/createSymbolicLink
                  target link
                  (make-array java.nio.file.attribute.FileAttribute 0)))
         ...
         (project-input-files input-root
                              (get-in tree [:options :exclude-directories] #{})))
        development-files
        (mapv
         (fn [module]
           (let [plan (get plans-by-module module)
                 relative (.relativize input-path (.toPath ^File (:file plan)))
                 relative-string (str/replace (str relative) "\\" "/")
                 bootstrap? (= module entry-module)
                 source
                 (runtime/development-source
                  module
                 (when bootstrap?
                    {:bootstrap-declaration entry-declaration
                     ...
                     (str "_ = @import("
                          (emitter/emit-expr loader-import)
                          ").start();")}))
                 source (restore-module-imports module-imports module source)
                 source (if format?
                          (format-materialized-zig
                           zig "editor-tree" source output-root relative-string)
                          source)
                 target (.normalize (.resolve output-path relative))]
             (Files/writeString
              target source StandardCharsets/UTF_8
        ...
        {:input-root (.getAbsolutePath input-root)
         ...
         :entry-declaration (str entry-declaration)
         :loader-relative-path loader-relative-path
         :copied-file-count (count copied)
         :development-module-count (count development-files)
         :development-files development-files
         :elapsed-ms (/ (- (System/nanoTime) started) 1e6)}]
    (record-conversion! result)))
...
(defn- import-target
  ...
  (let [relative-path (.getCanonicalPath
    ...
    (or (when build-module-path (get plan-by-path build-module-path))
        ...
        (when-not (str/ends-with? import-name ".zig")
          (let [target (get module-index import-name)]
            ;; A bare build-module name that happens to equal the current
            ;; file's basename is not evidence of a self import. External
            ;; modules commonly collide this way (`sentry.zig` imports the
            ;; build module `sentry`). Only an explicit captured Build.Module
            ;; edge may identify the current file as that named module.
            (when-not (= (some-> target :file .getCanonicalPath)
                         (some-> plan :file .getCanonicalPath))
              target))))))
